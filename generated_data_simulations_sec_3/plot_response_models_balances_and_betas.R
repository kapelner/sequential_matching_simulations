...
plot_response_model = function(nsim, betaT, final_reservoir_size){
	for (j in 1 : p){
		pdf(file = paste("response_", randomization_type, "_", response_model, "_n_", n, "_nsim_", nsim, "_alpha_", prob_match_cutoff_alpha, "_x_", j, ".pdf", sep = ""))
		
		plot(Xy[, j], Xy$y, type = "n", 
				main = paste("x vs y b:", round(betaT, 2), 
						"\nn =", n, "alpha =", prob_match_cutoff_alpha, "response:", response_model,
